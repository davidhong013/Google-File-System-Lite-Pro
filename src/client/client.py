...
def create_file(file_path):
    try:
        ...
        with grpc.insecure_channel(master) as channel:
            stub = gfs_pb2_grpc.MasterServerToClientStub(channel)
            request = gfs_pb2.FileRequest(filename=file_path)  # Make sure this matches your .proto definition
            file_response = stub.CreateFile(request)  # Ensure this is a string returned by the method
        ...
        for chunk_servers in data[1:]:
            with grpc.insecure_channel(chunk_servers) as channel:
                ...
                chunk_request = gfs_pb2.ChunkRequest(chunk_id = file_path + '|' + chunk_index)
