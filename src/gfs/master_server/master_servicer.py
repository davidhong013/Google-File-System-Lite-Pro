...
class MasterServerToClientServicer(gfs_pb2_grpc.MasterServerToClientServicer):
    ...
    def VerifyLease(self, request, context):
        ...
        if answer == 'Error':
            return gfs_pb2.FileResponse(success = False, message="Lease cannot be verified")
    ...
    def AppendFile(self, request, context):
        ...
        if answer == 'Error':
            return gfs_pb2.FileResponse(success=False, message="write failed, such file does not exist the file system")
        return gfs_pb2.FileResponse(success = True, message = answer)
