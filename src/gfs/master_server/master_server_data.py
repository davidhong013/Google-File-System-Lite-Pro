            ...
            for chunk_object in chunk_arr:
                ...
                with grpc.insecure_channel(chunk_address) as channel:
                    ...
                    request = gfs_pb2.FileRequest(
                        filename=MasterServer.path_name_transform(file)
                    )
                    response = stub.GetNumOfRead(request)
                if not response or not response.success:
                    print(
                        "something happened in dynamic allocation, reading stats stage",
                        file=sys.stderr,
                    )
                    sys.exit(1)
        ...
        for index in range(nums_to_deal):
            ...
            with file_object.file_lock:
                ...
                with grpc.insecure_channel(chunk_arr[0].chunk_address) as channel:
                    stub = gfs_pb2_grpc.ChunkServerToMasterServerStub(channel)
                    request = gfs_pb2.DuplicateRequest(
                        source=chunk_arr[0].chunk_address,
                        destination=sample_address,
                        file_name=MasterServer.path_name_transform(file),
                    )
                    response = stub.DuplicateFile(request)
                if not response or not response.success:
                    print(
                        "something happened in dynamic allocation, allocation stage",
                        file=sys.stderr,
                    )
                    sys.exit(1)
